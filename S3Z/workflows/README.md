...
```bash
python3 S3Z/workflows/mapper/run_mapper_jsonl.py \
...
```bash
python3 S3Z/workflows/e2e_runner/run_senzing_e2e.py \
  /path/to/production_output.jsonl
```
...
```bash
python3 S3Z/workflows/testing/run_management_tests.py \
...
```bash
python3 S3Z/tools/run_sample_to_management.py --input-json /path/to/input.json
```

If no `--input-json` is provided, the command generates a realistic sample first:

```bash
python3 S3Z/tools/run_sample_to_management.py --records 500
```
