...
def main() -> int:
    ...
    if not load_ok:
        if args.enable_load_chunk_fallback and args.load_chunk_size > 0:
            runtime_warnings.append(
                f"load_records standard retries failed; trying chunked fallback (chunk_size={args.load_chunk_size})."
            )
            chunk_ok, chunk_steps, chunk_warnings, chunks_attempted, records_attempted = run_chunked_load_fallback(
                ...
                engine_config_json=runtime_engine_config,
                license_string=license_string,
                keep_chunk_files=args.keep_load_chunk_files,
            ...
            if chunk_ok:
                load_ok = True
                runtime_warnings.append(
                    f"chunked load fallback succeeded ({chunks_attempted} chunks, {records_attempted} records)."
                )
            else:
                runtime_warnings.append(
                    f"chunked load fallback failed after {chunks_attempted} chunks."
                )
