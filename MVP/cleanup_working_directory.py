#!/usr/bin/env python3
"""Cleanup utility for MVP working directories.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Clean generated MVP runtime/output artifacts.")
    parser.add_argument(
        "--mvp-root",
        default=str(Path(__file__).resolve().parent),
        help="MVP root directory (default: script directory)",
    )
    parser.add_argument(
        "--runtime-dir",
        default=None,
...
def main() -> int:
    ...
    print(f"[{mode}] MVP root: {mvp_root}")
