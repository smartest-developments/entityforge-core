...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run core pipeline from source JSON to timestamped output artifacts.")
    ...
    parser.add_argument(
        "--runtime-dir",
        default=None,
        help="Optional runtime directory (default: temporary directory outside core)",
    )
    parser.add_argument(
        "--output-label",
        default=None,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "If omitted, auto-detects core/.secrets/g2.lic_base64 and core/license/g2.lic_base64."
        ),
    )
...
def relocate_output_bundle_to_run_dir(output_run_dir: Path, run_dir: Path, copied: dict[str, str]) -> tuple[Path, dict[str, str]]:
    """Move the final output bundle under the runtime run directory."""
...
def main() -> int:
    ...
    mapper_script = app_root / "partner_json_to_senzing.py"
    e2e_script = app_root / "run_senzing_end_to_end.py"
