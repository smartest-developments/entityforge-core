#!/usr/bin/env python3
"""Export a simple record_id/cluster_id CSV from source input records.

The output matches the mapper's technical RECORD_ID assignment:
- record_id is the 1-based row index in the input file
- cluster_id is the source IPG ID resolved from the same row
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Export record_id/cluster_id CSV using the same record_id logic as the mapper."
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Optional output CSV path. Default: <input_stem>__cluster_record_ids.csv",
    )
    parser.add_argument(
        "--array-key",
        default=None,
...
def default_output_path(input_path: Path) -> Path:
    return input_path.with_name(f"{input_path.stem}__cluster_record_ids.csv")
