#!/usr/bin/env python3
"""Build a static, local-first dashboard data file from core output runs."""
...
<html lang="en">
  ...
  <body>
    ...
    <ol>
      ...
      <li>Use <code>python3 app/verify_dashboard_metrics.py</code> for cross-check report output.</li>
    </ol>
  </body>
</html>
""",
...
def main() -> int:
    ...
    verifier = mvp_root / "app" / "verify_dashboard_metrics.py"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build management dashboard data from core outputs")
    parser.add_argument("--output-root", default="output", help="Output root containing timestamped run folders")
    parser.add_argument(
        "--dashboard-dir",
        default="dashboard",
        help="Dashboard directory under core root (self-contained, ready to open)",
    )
    parser.add_argument(
        "--skip-tests",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Streamlit assets directory under core root to refresh from dashboard assets",
    )
    parser.add_argument(
        "--skip-streamlit-sync",
        action="store_true",
...
def resolve_dashboard_template_dir(mvp_root: Path, dashboard_dir: Path) -> Path | None:
    ...
    Supports both:
    - standard layout: <core>/dashboard
    - production copied layout: <repo_root>/core/dashboard
    """
    candidates = [
        (mvp_root / "dashboard").resolve(),
        (mvp_root.parent / "core" / "dashboard").resolve(),
    ]
